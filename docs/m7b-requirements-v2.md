...
承認者: ショウゴさん（2026-08-16承認。裁定①③④明示承認、②はポート配分承認に包含）
...
## 7. 拡張性の指針（2026-08-16裁定時に追記）

Funnelの公開枠は**ポート三つだが、アプリ三つではない**。拡張軸は二つある。

1. **パス方式**: 同一ポートに `--set-path` で複数アプリをマウントできる（例: 443の `/app2` を別ポートへ転送）。アプリ側がサブパス配下で動くことが条件（Viteは`base`設定で対応可。ルート前提のPWAは不向き）
2. **ノード方式**: 三ポート制限は**端末ごと**。Surface Pro 7等を常時稼働ノードにすれば、別ホスト名でさらに三枠を得られる

プライバシーの序列: Funnelの中継サーバーはトラフィックを復号せず、TLS終端はノード側で行われる（Tailscale社は内容を読めない）。この性質はポート番号に依らず443・8443・10000で同一。対してCloudflare TunnelはTLSがCloudflareエッジで終端されるため、事業者が内容を技術的に読める。

**乗り換えの判断基準**: 以下のいずれかが現実化した時点でCloudflare Tunnel（無料・要独自ドメイン・サブドメイン無制限）を第一候補として検討する。それまでは変えない。
- (a) パス方式・ノード方式でも公開アプリが収まらなくなった
- (b) Funnelの帯域制限（非設定可能）が実用を妨げた
- (c) ルート前提のアプリを枠数以上公開する必要が生じた

移行する場合も、私室系（chat-pwa等の親密領域）はプライバシー序列によりFunnelに残し、公開系のみを移す二経路構成とする。

## 改訂履歴

- v2.1（2026-08-16）: 発注者承認を記帳（①③④明示・②包含）。§7拡張性の指針を追記
